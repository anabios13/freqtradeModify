...
class DockerComposeGenerator:
    ...
    def generate_volume_names(self, strategy_name: str) -> Dict[str, str]:
        ...
        # Создаем короткое имя для тома (максимум 20 символов)
        base_name = strategy_name.lower().replace(' ', '_').replace('-', '_')
        if len(base_name) > 15:
            base_name = base_name[:15]
        
        return {
            'models': f"{base_name}_models",
            'db': f"{base_name}_db",
            'logs': f"{base_name}_logs",
            'freqai': f"{base_name}_freqai",
            'data': f"{base_name}_data"
        }
    ...
    def generate_freqtrade_service(self, strategy_name: str, config_path: Path, is_freqai: bool) -> str:
        ...
        if is_freqai:
            command = f'trade --config /app/user_data/dryrun_configs/{config_path.name} --strategy {strategy_name} --freqaimodel LightGBMRegressor'
        else:
            command = f'trade --config /app/user_data/dryrun_configs/{config_path.name} --strategy {strategy_name}'
...
"""
    ...
    def generate_volumes_section(self, strategies: List[Tuple[str, Path]]) -> str:
        ...
        volumes = set()  # Используем set для избежания дублирования
        ...
        # Добавляем тома для каждой стратегии
        for strategy_name, _ in strategies:
            strategy_volumes = self.generate_volume_names(strategy_name)
            volumes.update(strategy_volumes.values())
    ...
    def create_docker_config(self, strategy_name: str, original_config_path: Path) -> Path:
        ...
        # Используем оригинальную папку dryrun_configs
        configs_dir = self.base_dir / "user_data" / "dryrun_configs"
        ...
        # Сохраняем исправленную конфигурацию в dryrun_configs
    ...
    def fix_config_for_docker(self, config: dict, strategy_name: str) -> dict:
        ...
        # Всегда добавляем db_url для правильной работы с существующими базами данных
        db_name = strategy_name
        docker_config["db_url"] = f"sqlite:///user_data/dryrun_db/{db_name}.sqlite"
        print(f"      🔧 Добавлен путь к БД: {docker_config['db_url']}")
...
services:
  ...
  data-aggregator:
    ...
    volumes:
      - .:/app:ro
      - shared_models:/app/user_data/models:ro
      - aggregated_data:/tmp/aggregated_data:rw"""
        
        # Добавляем общую папку для баз данных
        content += f"\n      - shared_databases:/app/user_data/dryrun_db:ro"
  ...
  streamlit:
    ...
    volumes:
      - .:/app:ro
      - aggregated_data:/tmp/aggregated_data:ro
    ports:
      - "8501:8501"
    environment:
      - PYTHONPATH=/app
...
if [ ! -f "$COMPOSE_FILE" ]; then
    ...
    echo "Запустите сначала: python scripts/generate_docker_compose_fixed.py"
    exit 1
fi
