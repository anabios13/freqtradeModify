...
class DockerComposeGenerator:
    ...
    def find_matching_config(self, strategy_name: str, configs: List[Tuple[str, Path]]) -> Path:
        ...
        for config_name, config_path in configs:
            if strategy_name.lower() in config_name.lower() or config_name.lower() in strategy_name.lower():
                return config_path
    ...
    volumes:
      ...
      - {volumes['db']}:/app/user_data/databases
  ...
  data-aggregator:
    ...
    volumes:
      - .:/app:ro"""
        ...
        # Добавляем доступ к базам данных только активных стратегий
        for strategy_name, _, _ in active_strategies:
            volumes = self.generate_volume_names(strategy_name)
            content += f"\n      - {volumes['db']}:/app/user_data/databases:ro"
...
"""
    ...
    def create_missing_configs(self, strategies: List[Tuple[str, Path]], configs: List[Tuple[str, Path]]) -> None:
        """Создает недостающие конфигурации для стратегий"""
        ...
        for strategy_name, strategy_path in strategies:
            ...
            if not config_path:
                print(f"  📝 Создание конфигурации для {strategy_name}...")
                self.create_config_template(strategy_name, strategy_path)
    
    def create_config_template(self, strategy_name: str, strategy_path: Path) -> None:
        """Создает шаблон конфигурации для стратегии"""
        config_dir = self.base_dir / "user_data" / "configs"
        config_dir.mkdir(exist_ok=True)
        
        config_path = config_dir / f"config_{strategy_name}.json"
        ...
        # Добавляем FreqAI конфигурацию если нужно
        if is_freqai:
            config_template["freqai"] = {
                "enabled": True,
                "purge_old_models": False,
                "train_period_days": 30,
                "backtest_period_days": 7,
                "identifier": f"{strategy_name}FreqAI",
                "feature_parameters": {
